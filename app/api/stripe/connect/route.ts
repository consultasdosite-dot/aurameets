...
export async function POST(request: NextRequest) {
  try {
    ...
    if (therapistError) {
      ...
      return NextResponse.json(
        { error: "Não foi possível localizar o perfil do terapeuta." },
        { status: 500 },
      );
    }
    ...
    if (!stripeAccountId) {
      const account = await stripe.accounts.create({
        type: "express",
        country: therapist.country_code || "BR",
        email: therapist.email || user.email || undefined,
      ...
      if (updateError) {
        console.error(
          "Erro ao salvar conta Stripe no terapeuta:",
          updateError,
        );
    ...
    const accountLink = await stripe.accountLinks.create({
      ...
      refresh_url: `${origin}/dashboard-terapeuta/stripe?refresh=1`,
      return_url: `${origin}/dashboard-terapeuta/stripe?success=1`,
    ...
    return NextResponse.json(
      {
        error: "Não foi possível iniciar a conexão com a Stripe.",
        details:
