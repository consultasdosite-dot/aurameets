...
function statusLabel(status: string | null) {
  switch (status) {
    ...
    default:
      return status?.toUpperCase() || "SEM STATUS";
  }
}
    ...
    <main className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-slate-900/60 p-6 sm:p-8">
        ...
        <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-400">
          Acompanhe as compras realizadas pela
          plataforma, os valores destinados aos
          terapeutas e a comissão bruta do
          AuraMeets.
        </p>
      </section>
      ...
      {checkoutsAbertos > 0 && (
        <section className="rounded-2xl border border-amber-400/20 bg-amber-500/10 px-5 py-4">
          <p className="text-sm font-semibold text-amber-200">
            Existem {checkoutsAbertos} checkout(s)
            criado(s) que ainda não estão marcados
            como pagamento concluído.
          </p>
        </section>
      )}
      ...
      <section className="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60">
        <div className="border-b border-white/10 p-6">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Últimos {pagamentos.length} registros
            encontrados.
          </p>
        </div>
      ...
      <section className="rounded-2xl border border-blue-400/20 bg-blue-500/10 px-5 py-4">
        <p className="text-sm leading-6 text-blue-200">
          ...
          exibida acima corresponde à comissão
          bruta registrada pelo AuraMeets. As
          tarifas cobradas pela Stripe ainda não
          estão armazenadas na tabela de
          pagamentos.
        </p>
      </section>
    </main>
